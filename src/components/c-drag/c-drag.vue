...
<template>
    ...
    >
      <view class="fish-drag" :style="{ height: `${wrapHeight}px` }">
        ...
        >
          <!-- uni app vue3 不支持 v-bind https://github.com/dcloudio/uni-app/issues/3330 -->
          <slot name="item" :item="item.data" />
          <view
            catch-move
            @longpress="(e: any) => onLongPress(e, index)"
  ...
  import { defineProps, withDefaults, defineEmits, reactive, computed, watch, getCurrentInstance } from 'vue';
  ...
  const emit = defineEmits<{
    ...
    // eslint-disable-next-line no-unused-vars
    (e: 'dragstart'|'dragend', event: TouchEvent): void;
  }>();
  ...
  const onLongPress = (event: TouchEvent, originIndex: number) => {
  ...
  const onTouchStart = () => {
    uni.createSelectorQuery().in(thisInstance).select(`#${props.id}`).scrollOffset((res) => {
      _baseData.scrollTop = (res as UniApp.NodeInfo).scrollTop!;
      startMove();
    }).exec()
  ...
  const onTouchEnd = (event: TouchEvent) => {
    emit('dragend', event as any as TouchEvent);
    reset();
  ...
  const render = (init?: 'init'|any) => {
    ...
    const getItemRect = () => {
      return new Promise((resolve, reject) => {
        query.select(`#${props.id} .fish-drag-item`).boundingClientRect((res) => resolve(res)).exec();
      })
    };
    ...
    setTimeout(async () => {
      ...
      const [container, item] = await Promise.all([getContainerRect(), getItemRect()]) as UniApp.NodeInfo[];
  ...
  onReady(() => {
    render('init');
    _baseData.windowHeight = uni.getSystemInfoSync().windowHeight;
  });
  
  const isEqualListData = (l1: IPropsListItem[], l2: IPropsListItem[]): boolean => {
    const isEqual = (a: any, b: any): boolean => { // _.isEqual 简单判断
